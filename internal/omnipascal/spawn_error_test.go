...
func TestIsPermissionLikeError(t *testing.T) {
	tests := []struct {
		...
		{name: "access denied string", err: errors.New("Acesso negado"), want: true},
...
func TestNewProcessStartErrorIncludesHintsForPermissionErrors(t *testing.T) {
	err := newProcessStartError("OmniPascal server", `C:\tmp\OmniPascalServer.exe`, errors.New("Acesso negado"))
	message := err.Error()

	if !strings.Contains(message, "failed to start OmniPascal server") {
